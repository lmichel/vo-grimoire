...
function addAttachements(attachements,numMail){
    ...
    if (attachements){
        ...
        for (const [key, value] of Object.entries(JSON.parse(attachements))){
            ...
            temp += "<a style='border-color: transparent;display: inline-block' class='btn btn-outline-secondary' title='Click here to download "+nom+"' href='data:"+type+";"+encode+','+encodeURI(value).replace(/</g,"&lt").replace(/>/g,"&gt")+"' download='"+nom+
                "' ><span class=\"fa fa-download\"> Download </span></a>"
            a+= temp + "</div>"
        }
    }
...
function addImageAttachementModal(type,encode,nom,value,fullName,id){
    return "<button style='border-color: transparent;display: inline-block' title='Display in a modal box "+fullName+"' type=\"button\" class=\"btn btn-outline-secondary btn_url_mails\" data-target=\"#"+id+"\" data-toggle=\"modal\">" +
